...
def shouldSwingCorner(robotPosition, bullPosition, corralPositions):
    """ Determines if the robot's optimal move is swinging the corner of the corral \n
    Parameters: \n
    robotPosition: list containing x and y value of robot \n
    bullPosition: list containing x and y value of bull \n
    corralPosition: a set containing only one list that is the position of the target \n
    returns \n
    Booleans"""
    for p in corralPositions:
        open = [p[0]-2, p[1]]
        l_corner = [p[0]-2, p[1]-1]
        r_corner = [p[0]-2, p[1]+1]
    if bullPosition == open and (robotPosition == l_corner or robotPosition == r_corner):
        return True
    return False


def swingCorner(robotPosition, corralPositions):
    """ Decides which corner the robot swings and moves the robot \n
        Parameters: \n
        robotPosition: list containing x and y value of robot \n
        corralPosition: a set containing only one list that is the position of the target \n
         returns \n
         updates the robotPosition \n
         void
         """
    for p in corralPositions:
        l_corner = [p[0] - 2, p[1] - 1]
